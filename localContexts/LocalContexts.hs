...
-- TODO: build the update logical directly into the semantics
...
entailsHelper cp cf =
  ContT $ \k ->
    Identity $ cf >>- \f -> cp >>- \p -> liftM2 (==>) p (runIdentity $ k f)

andHelper' :: Cont (S -> T) a -> Cont (S -> T) (S -> T) -> Cont (S -> T) a
andHelper' cf cp =
  ContT $ \k ->
    Identity $ cf >>- \f -> cp >>- \p -> liftM2 (&&) (runIdentity $ k f) p
...
cf |= cg =
  ContT $ \k ->
    Identity $
    cf >>- \f ->
      cg >>- \g -> liftM2 (==>) (runIdentity $ k f) (runIdentity $ k g)
...
--
leave :: E -> S -> T
leave A = toCharFunc [1, 2, 3, 4, 5, 6, 7]
leave B = toCharFunc []
leave C = toCharFunc [0, 4, 5, 7]
...
instance Functor LC where
  fmap f (LC a) = LC $ \p -> fmap f (a p) `andHelper'` p

instance Applicative LC where
  pure a = LC $ \p -> pure a `andHelper'` p
  (LC f) <*> (LC x) = LC $ \p -> f p <*> x p `andHelper'` p

instance Monad LC where
  return = pure
  (LC x) >>= f = LC $ \p -> (x p >>= ($ p) . runLC . f) `andHelper'` p

-- >>> toSet [1..10] (lower . ($ (return $ toCharFunc [0..10])) . runLC $ (return :: a -> LC a) ((liftM2 (&&)) ((return A) >>= leave) ((return A) >>= boy)))
-- update
(+++) :: LC (S -> T) -> LC (S -> T) -> LC (S -> T)
(LC p) +++ (LC q) = LC $ \r -> p . q $ r

-- toy example of a presuppositional predicate.
...
leavePresupp =
  LC $ \r ->
    ContT $ \k ->
      k
        (\x s ->
           null [w | w <- [0 .. 10], lower r w && not (boy x w)] && leave x s)
