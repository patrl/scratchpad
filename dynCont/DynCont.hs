...
abstract n (ReaderT f) = ReaderT (\g ->
...
drefIntro n x cp = CCP $ \k ->
                         ReaderT $ \g ->
                                     runReaderT (cp >>- k) (modify g n x)

dynamicAbs :: Var -> CCP T -> E -> CCP T
dynamicAbs n cp x = CCP $ \k ->
                            ReaderT $ \g ->
                                        runReaderT (cp >>- k) (modify g n x)
...
dynIndef n cp = CCP $ \k ->
  ex n $ (cp `ap` liftG (pro n)) >>- liftM2 (&&) (k $ pro n)

dynUniv :: Var -> CCP (E -> T) -> CCP E
dynUniv n cp = CCP $ \k ->
  forall n $ (cp `ap` liftG (pro n)) >>- liftM2 (==>) (k $ pro n)
...
showAppContext ccp = toSet assignments $ fmap runIdentity $ runReaderT $ (>>-) ccp id

-- >>> toSet assignments $ fmap runIdentity $ runReaderT $ (flip (>>-)) id $ ((pure leave) <*> (liftG (pro 0))) `conjDyn` (exDyn 0 $ (pure boy) <*> (liftG (pro 0)))
